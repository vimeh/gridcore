...
test.describe("Initial Render Tests", () => {
  test("grid should render immediately without user interaction", async ({ page }) => {
    ...
    const canvasHasContent = await page.evaluate(() => {
      const canvas = document.querySelector("canvas.grid-canvas") as HTMLCanvasElement;
  ...
  test("active cell border should be visible on initial load", async ({ page }) => {
    await page.goto("/");
    ...
    const hasActiveCellBorder = await page.evaluate(() => {
      ...
      for (let i = 0; i < data.length; i += 4) {
        ...
        if (data[i] < 50 && data[i + 1] > 100 && data[i + 1] < 150 && data[i + 2] > 200) {
          return true;
        }
      }
