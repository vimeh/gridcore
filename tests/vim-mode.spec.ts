...
test.describe("Vim Mode", () => {
  ...
  test("should transition between vim modes correctly", async ({ page }) => {
    ...
    await expect(
      page.locator(".mode-indicator").filter({ hasText: "i/a to insert" }),
  ...
  test("should position cursor correctly with vim commands", async ({
    page,
  }) => {
  ...
  test("should show visual mode", async ({ page }) => {
    ...
    await expect(
      page.locator(".mode-indicator").filter({ hasText: "hjkl to select" }),
