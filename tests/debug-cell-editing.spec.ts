...
test.describe("Debug Cell Editing", () => {
  ...
  test("debug Enter key editing", async ({ page }) => {
    ...
    const editorVisible = await page.locator(".cell-editor-overlay").isVisible().catch(() => false);
    console.log("Editor visible after Enter:", editorVisible);
    
    // Check current mode
    const modeText = await page.locator(".mode-text").textContent().catch(() => "not found");
    ...
    if (editorVisible) {
      const editorValue = await page.locator(".cell-editor-overlay textarea").inputValue().catch(() => "");
