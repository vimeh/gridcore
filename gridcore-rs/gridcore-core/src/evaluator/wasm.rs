...
impl EvaluationContext for WasmEvaluationContext {
    fn get_cell_value(&self, address: &CellAddress) -> crate::Result<CellValue> {
        ...
        let result = self.get_cell_value_callback
            .call1(&JsValue::NULL, &addr_str)
            .map_err(|e| SpreadsheetError::FormulaError(
                format!("Error calling JS callback: {:?}", e)
            ))?;
        
        // Convert JS value to CellValue
        CellValue::from_js(result)
            .map_err(|e| SpreadsheetError::FormulaError(
                format!("Error converting JS value: {:?}", e)
            ))
    }
