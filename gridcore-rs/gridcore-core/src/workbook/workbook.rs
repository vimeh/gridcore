...
impl Workbook {
    ...
    pub fn add_sheet(&mut self, sheet: Sheet) -> Result<()> {
        ...
        if self.sheets.contains_key(&name) {
            return Err(SpreadsheetError::InvalidOperation(format!("Sheet '{}' already exists", name)));
        }
    ...
    pub fn remove_sheet(&mut self, name: &str) -> Result<Sheet> {
        if self.sheets.len() <= 1 {
            return Err(SpreadsheetError::InvalidOperation("Cannot remove the last sheet".to_string()));
        }
        
        let sheet = self.sheets.remove(name)
            .ok_or_else(|| SpreadsheetError::InvalidOperation(format!("Sheet '{}' not found", name)))?;
        ...
        self.global_named_ranges.retain(|_, (sheet_name, _)| sheet_name != name);
    ...
    pub fn rename_sheet(&mut self, old_name: &str, new_name: impl Into<String>) -> Result<()> {
        ...
        if self.sheets.contains_key(&new_name) {
            return Err(SpreadsheetError::InvalidOperation(format!("Sheet '{}' already exists", new_name)));
        }
        
        let mut sheet = self.sheets.remove(old_name)
            .ok_or_else(|| SpreadsheetError::InvalidOperation(format!("Sheet '{}' not found", old_name)))?;
    ...
    pub fn copy_sheet(&mut self, source_name: &str, target_name: impl Into<String>) -> Result<()> {
        ...
        if self.sheets.contains_key(&target_name) {
            return Err(SpreadsheetError::InvalidOperation(format!("Sheet '{}' already exists", target_name)));
        }
        
        let source_sheet = self.sheets.get(source_name)
            .ok_or_else(|| SpreadsheetError::InvalidOperation(format!("Sheet '{}' not found", source_name)))?;
    ...
    pub fn move_sheet(&mut self, name: &str, new_index: usize) -> Result<()> {
        if !self.sheets.contains_key(name) {
            return Err(SpreadsheetError::InvalidOperation(format!("Sheet '{}' not found", name)));
        }
        
        let current_index = self.sheet_order.iter().position(|n| n == name)
            .ok_or_else(|| SpreadsheetError::InvalidOperation("Sheet not in order list".to_string()))?;
        
        if new_index >= self.sheet_order.len() {
            return Err(SpreadsheetError::InvalidOperation("Invalid sheet index".to_string()));
        }
    ...
    pub fn active_sheet(&self) -> Option<&Sheet> {
        self.active_sheet.as_ref().and_then(|name| self.sheets.get(name))
    }

    /// Get the active sheet mutably
    pub fn active_sheet_mut(&mut self) -> Option<&mut Sheet> {
        self.metadata.modified_at = Utc::now();
        self.active_sheet.as_ref().and_then(|name| self.sheets.get_mut(name))
    }
    ...
    pub fn add_global_named_range(
        ...
        if !self.sheets.contains_key(&sheet_name) {
            return Err(SpreadsheetError::InvalidOperation(format!("Sheet '{}' not found", sheet_name)));
        }
        
        self.global_named_ranges.insert(name.into(), (sheet_name, addresses));
        self.metadata.modified_at = Utc::now();
        Ok(())
    }
    ...
    pub fn get_global_named_range(&self, name: &str) -> Option<(&str, &Vec<CellAddress>)> {
        self.global_named_ranges.get(name)
    ...
    pub fn parse_sheet_reference(&self, reference: &str) -> Result<(String, CellAddress)> {
        ...
        if parts.len() != 2 {
            return Err(SpreadsheetError::InvalidOperation(format!("Invalid sheet reference: {}", reference)));
        }
        ...
        let address = CellAddress::from_a1(parts[1])
            .map_err(|_| SpreadsheetError::InvalidOperation(format!("Invalid cell address: {}", parts[1])))?;
    ...
    pub fn set_cell_value(&mut self, sheet_name: &str, address: &CellAddress, cell: Cell) -> Result<()> {
        let sheet = self.sheets.get(sheet_name)
            .ok_or_else(|| SpreadsheetError::InvalidOperation(format!("Sheet '{}' not found", sheet_name)))?;
...
mod tests {
    ...
    fn test_copy_sheet() {
        ...
        sheet.set_cell(&CellAddress::new(0, 0), Cell::new(CellValue::Number(42.0))).unwrap();
    ...
    fn test_global_named_ranges() {
        ...
        workbook.add_global_named_range("MyRange", "Sheet1", range.clone()).unwrap();
