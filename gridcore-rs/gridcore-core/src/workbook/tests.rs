...
mod workbook_integration_tests {
    use crate::workbook::{Workbook, Sheet, SheetManager, SheetProperties};
    ...
    fn test_cross_sheet_references() {
        ...
        sheet1.set_cell(&CellAddress::new(0, 0), Cell::new(CellValue::Number(10.0))).unwrap();
        sheet1.set_cell(&CellAddress::new(1, 0), Cell::new(CellValue::Number(20.0))).unwrap();
    ...
    fn test_workbook_metadata() {
        ...
        metadata.custom_properties.insert("Department".to_string(), "Engineering".to_string());
        
        // Verify metadata
        let metadata = workbook.metadata();
        assert_eq!(metadata.title, "My Spreadsheet");
        assert_eq!(metadata.author, Some("Test Author".to_string()));
        assert_eq!(metadata.custom_properties.get("Department"), Some(&"Engineering".to_string()));
    }
    ...
    fn test_named_ranges() {
        ...
        workbook.add_global_named_range("GlobalData", "Sheet1", range.clone()).unwrap();
    ...
    fn test_sheet_cloning() {
        ...
        // Add some data
        original.set_cell(&CellAddress::new(0, 0), Cell::new(CellValue::Number(42.0))).unwrap();
        ...
        assert_eq!(
            cloned.get_cell(&CellAddress::new(0, 0)).unwrap().get_computed_value(),
