...
impl FormulaAdjuster for DefaultFormulaAdjuster {
    fn adjust_formula(
        ...
        let re = Regex::new(r"\$?[A-Z]+\$?\d+").map_err(|e| {
            SpreadsheetError::InvalidOperation(format!("Regex error: {}", e))
        })?;
...
mod tests {
    ...
    fn test_adjust_relative_reference() {
        ...
        let result = adjuster.adjust_formula("=A1+B1", &from, &to, FillDirection::Down).unwrap();
    ...
    fn test_adjust_absolute_reference() {
        ...
        let result = adjuster.adjust_formula("=$A$1+B1", &from, &to, FillDirection::Down).unwrap();
    ...
    fn test_adjust_mixed_reference() {
        ...
        let result = adjuster.adjust_formula("=$A1+A$1", &from, &to, FillDirection::Down).unwrap();
    ...
    fn test_parse_cell_reference() {
        ...
        assert_eq!(adjuster.parse_cell_reference("A1"), Some((false, 0, false, 1)));
        assert_eq!(adjuster.parse_cell_reference("$A1"), Some((true, 0, false, 1)));
        assert_eq!(adjuster.parse_cell_reference("A$1"), Some((false, 0, true, 1)));
        assert_eq!(adjuster.parse_cell_reference("$A$1"), Some((true, 0, true, 1)));
        assert_eq!(adjuster.parse_cell_reference("AA10"), Some((false, 26, false, 10)));
    }
    ...
    fn test_complex_formula_adjustment() {
        ...
        let result = adjuster.adjust_formula(
