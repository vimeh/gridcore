...
mod fill_integration_tests {
    use crate::fill::{
        CellRange, FillDirection, FillEngine, FillOperation, PatternType, FormulaAdjuster,
    ...
    fn test_formula_adjustment_relative_refs() {
        ...
        let adjusted = adjuster.adjust_formula(formula, &from, &to, FillDirection::Down).unwrap();
    ...
    fn test_formula_adjustment_mixed_refs() {
        ...
        let adjusted = adjuster.adjust_formula(formula, &from, &to, FillDirection::Right).unwrap();
