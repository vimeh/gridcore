...
mod references_integration_tests {
    use crate::references::{
        Reference, ReferenceType, StructuralOperation, adjuster::ReferenceAdjuster,
        detector::ReferenceDetector, parser::ReferenceParser, tracker::ReferenceTracker,
    };
    ...
    fn test_parse_complex_formula() {
        ...
        let has_range = refs
            .iter()
            .any(|r| matches!(r.ref_type, ReferenceType::Range(_, _)));
        assert!(has_range);

        // Check for sheet reference
        let has_sheet = refs
            .iter()
            .any(|r| matches!(r.ref_type, ReferenceType::Sheet(_, _)));
        assert!(has_sheet);
    }
    ...
    fn test_detect_affected_by_insert() {
        ...
        let abs_ref = Reference::new(ReferenceType::Absolute(0, 5), "$A$6".to_string());
        ...
        let rel_ref = Reference::new(ReferenceType::Relative(0, 5), "A6".to_string());
    ...
    fn test_tracker_dependency_management() {
        use crate::formula::{BinaryOperator, Expr};
    ...
    fn test_circular_reference_detection() {
        ...
        // Self-reference
        let self_ref = Reference::new(ReferenceType::Relative(0, 0), "A1".to_string());
        assert!(detector.is_circular(&CellAddress::new(0, 0), &self_ref));

        // Non-circular reference
        let other_ref = Reference::new(ReferenceType::Relative(1, 0), "B1".to_string());
