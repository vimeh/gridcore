...
impl EventAdapter {
    ...
    fn domain_to_spreadsheet_event(event: &DomainEvent) -> SpreadsheetEvent {
        match event {
            DomainEvent::CellChanged { address, old_value, new_value } => {
                SpreadsheetEvent::cell_updated(address, old_value.clone(), new_value.clone(), None)
            }
            DomainEvent::CellDeleted { address, .. } => {
                SpreadsheetEvent::cell_deleted(address)
            }
            ...
            DomainEvent::CalculationCompleted { affected_cells } => {
                let cell_strings: Vec<String> = affected_cells.iter().map(|addr| addr.to_string()).collect();
    ...
    fn subscribe(&mut self, handler: EventHandler) -> Result<String> {
        let handler_id = {
            let mut id_counter = self.next_handler_id.lock().map_err(|_| {
                crate::SpreadsheetError::InvalidOperation("Failed to lock handler ID counter".to_string())
            })?;
            *id_counter += 1;
