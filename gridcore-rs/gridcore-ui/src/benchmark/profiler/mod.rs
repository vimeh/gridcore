...
impl PerformanceProfiler {
    ...
    pub fn record_interaction(&mut self, event_type: &str, start_time: f64) {
        if self.is_recording {
            let end_time = self.performance.now();
            self.latency_tracker.record_interaction(event_type, start_time, end_time);
        }
    }
    ...
    pub fn mark(&mut self, name: &str) {
        if self.is_recording {
            ...
            leptos::logging::log!("Benchmark mark '{}' at {}ms", name, timestamp - self.start_time);
        }
    }
    ...
    pub fn measure(&mut self, name: &str, start_mark: &str, end_mark: &str) {
        if self.is_recording {
            leptos::logging::log!("Benchmark measure '{}' from '{}' to '{}'", name, start_mark, end_mark);
        }
    }
