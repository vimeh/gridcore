...
impl BenchmarkScenario for InitialRenderBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        metrics.custom_metrics.insert("data_generation_ms".to_string(), data_gen_time);
        metrics.custom_metrics.insert("data_load_ms".to_string(), load_time);
        metrics.custom_metrics.insert("viewport_render_ms".to_string(), render_time);
        metrics.custom_metrics.insert("time_to_interactive_ms".to_string(), metrics.end_time - metrics.start_time);
...
impl BenchmarkScenario for LargeGridRenderBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for (rows, cols, label) in &self.test_sizes {
            ...
            let density = (1000.0 / (*rows as f64 * *cols as f64)).min(0.1).max(0.001);
            let data_start = Self::now();
            let data = self.data_generator.generate_sparse_data(*rows, *cols, density);
            let data_gen_time = Self::now() - data_start;
            ...
            metrics.custom_metrics.insert(format!("{}_data_gen_ms", label), data_gen_time);
            metrics.custom_metrics.insert(format!("{}_load_ms", label), load_time);
            metrics.custom_metrics.insert(format!("{}_cells", label), data.len() as f64);
