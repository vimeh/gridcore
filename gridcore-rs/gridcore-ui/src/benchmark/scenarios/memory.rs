...
impl Default for MemoryGrowthBenchmark {
    fn default() -> Self {
        Self::new()
    }
}
...
impl BenchmarkScenario for MemoryGrowthBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for cycle in 0..self.cycles {
            ...
            metrics
                .custom_metrics
                .insert(format!("cycle_{}_time_ms", cycle), cycle_time);
            metrics
                .custom_metrics
                .insert(format!("cycle_{}_memory_growth_mb", cycle), mem_growth);
        }
        ...
        metrics
            .custom_metrics
            .insert("undo_redo_time_ms".to_string(), undo_time);
        ...
        if cell_count > 0 {
            ...
            metrics
                .custom_metrics
                .insert("bytes_per_cell".to_string(), bytes_per_cell);
        }

        metrics.cells_updated = self.operations_per_cycle * self.cycles;
...
impl Default for MemoryCleanupBenchmark {
    fn default() -> Self {
        Self::new()
    }
}
...
impl BenchmarkScenario for MemoryCleanupBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        metrics
            .custom_metrics
            .insert("phase1_load_time_ms".to_string(), phase1_time);
        metrics
            .custom_metrics
            .insert("phase1_memory_growth_mb".to_string(), phase1_growth);
        metrics
            .custom_metrics
            .insert("phase1_cells".to_string(), data.len() as f64);
        ...
        metrics
            .custom_metrics
            .insert("phase2_clear_time_ms".to_string(), phase2_time);
        metrics.custom_metrics.insert(
        ...
        metrics
            .custom_metrics
            .insert("phase3_switch_time_ms".to_string(), phase3_time);
        metrics.custom_metrics.insert(
        ...
        metrics.heap_peak = phase1_memory
            .max(phase2_memory_before_gc)
            .max(phase3_memory);
        ...
        metrics
            .custom_metrics
            .insert("cleanup_effectiveness_percent".to_string(), cleanup_ratio);
