...
impl BenchmarkScenario for CellEditBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for (pos, value) in self.test_positions.iter().zip(&self.test_values) {
            ...
            metrics.custom_metrics.insert(format!("nav_{}_ms", pos), nav_time);
            metrics.custom_metrics.insert(format!("enter_edit_{}_ms", pos), enter_edit_time);
            metrics.custom_metrics.insert(format!("value_{}_ms", pos), value_time);
            metrics.custom_metrics.insert(format!("commit_{}_ms", pos), commit_time);
        }
        ...
        metrics.custom_metrics.insert("bulk_paste_100_cells_ms".to_string(), paste_time);
...
impl SelectionBenchmark {
    pub fn new() -> Self {
        Self {
            test_ranges: vec![
                (CellAddress::new(0, 0), CellAddress::new(0, 0), "single_cell"),
                (CellAddress::new(0, 0), CellAddress::new(9, 9), "10x10_range"),
                (CellAddress::new(0, 0), CellAddress::new(99, 25), "100x26_range"),
                (CellAddress::new(0, 0), CellAddress::new(999, 99), "1000x100_range"),
            ],
        }
    }
}
...
impl BenchmarkScenario for SelectionBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for (start, end, label) in &self.test_ranges {
            ...
            metrics.custom_metrics.insert(format!("{}_select_ms", label), select_time);
            
            // Calculate cells in selection
            let cells_selected = ((end.row - start.row + 1) * (end.col - start.col + 1)) as f64;
            metrics.custom_metrics.insert(format!("{}_cells", label), cells_selected);
            ...
            metrics.custom_metrics.insert(format!("{}_clear_ms", label), clear_time);
        }
        ...
        metrics.custom_metrics.insert("multi_select_5_cells_ms".to_string(), multi_time);
        ...
        metrics.custom_metrics.insert("select_all_ms".to_string(), select_all_time);
