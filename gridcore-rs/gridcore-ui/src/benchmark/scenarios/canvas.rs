...
impl BenchmarkScenario for DrawCallBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for (rows, cols, label) in &self.cell_counts {
            leptos::logging::log!("Testing canvas rendering for {} ({} x {})", label, rows, cols);
            ...
            // Store metrics
            metrics.custom_metrics.insert(format!("{}_load_ms", label), load_time);
            metrics.custom_metrics.insert(format!("{}_grid_render_ms", label), grid_time);
            metrics.custom_metrics.insert(format!("{}_content_render_ms", label), content_time);
            metrics.custom_metrics.insert(format!("{}_selection_render_ms", label), selection_time);
            metrics.custom_metrics.insert(format!("{}_highlight_render_ms", label), highlight_time);
            
            // Estimate draw calls (this is approximate)
            let estimated_draw_calls = 
                1.0 +                      // Clear canvas
            ...
            metrics.custom_metrics.insert(format!("{}_estimated_draw_calls", label), estimated_draw_calls);
...
impl BenchmarkScenario for CanvasStateBenchmark {
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        // Store metrics
        metrics.custom_metrics.insert("transform_operations_ms".to_string(), transform_time);
        metrics.custom_metrics.insert("clipping_operations_ms".to_string(), clip_time);
        metrics.custom_metrics.insert("style_changes_ms".to_string(), style_time);
        ...
        if total_time > 0.0 {
            ...
            metrics.custom_metrics.insert("state_changes_per_ms".to_string(), changes_per_ms);
        }
