...
impl Default for SmoothScrollBenchmark {
    fn default() -> Self {
        Self::new()
    }
}
...
impl BenchmarkScenario for SmoothScrollBenchmark {
    ...
    fn warmup(&mut self, controller: Rc<RefCell<SpreadsheetController>>) {
        ...
        let data = self
            .data_generator
            .generate_numeric_grid(100, 50, 0.0, 1000.0);
...
impl Default for JumpNavigationBenchmark {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for (row, col) in &self.jump_positions {
            ...
            if !metrics.frame_times.is_empty() {
                ...
                metrics
                    .custom_metrics
                    .insert("max_jump_distance".to_string(), distance);
            }
            metrics
                .custom_metrics
                .insert("last_row".to_string(), *row as f64);
        }
...
impl Default for LargeDatasetScrollBenchmark {
    fn default() -> Self {
        Self::new()
    }
}
    ...
    fn run(&mut self, controller: Rc<RefCell<SpreadsheetController>>) -> BenchmarkResult {
        ...
        for (pattern_name, positions) in scroll_patterns {
            for row in positions {
                ...
                metrics
                    .custom_metrics
                    .insert(format!("{}_avg_ms", pattern_name), scroll_time);
            }
        }
