...
impl ResultsCollector {
    ...
    fn calculate_summary(&self) -> BenchmarkSummary {
        ...
        let all_fps: Vec<f64> = self.results
            .iter()
        ...
        let all_latencies: Vec<f64> = self.results
            .iter()
        ...
        let total_memory_growth: f64 = self.results
            .iter()
            .map(|r| r.metrics.memory_growth)
            .sum();
        
        // Calculate total duration
        let total_duration: f64 = self.results
            .iter()
    ...
    fn generate_suggestions(&self) -> Vec<String> {
        ...
        if summary.avg_fps < 50.0 {
            suggestions.push("Consider optimizing rendering performance - average FPS is below 50".to_string());
        }
        
        if summary.p95_latency > 150.0 {
            suggestions.push("Input latency is high - consider optimizing event handlers".to_string());
        }
        
        if summary.total_memory_growth > 50.0 {
            suggestions.push("Significant memory growth detected - check for memory leaks".to_string());
        }
        
        // Check for specific scenario issues
        let scenario_groups = self.group_by_scenario();
        for (scenario, results) in scenario_groups {
            let avg_fps: f64 = results.iter()
                .map(|r| r.metrics.fps_avg)
                .filter(|&fps| fps > 0.0)
    ...
    fn get_unique_scenarios(&self) -> Vec<String> {
        let mut scenarios: Vec<String> = self.results
            .iter()
    ...
    fn group_by_scenario(&self) -> HashMap<String, Vec<&BenchmarkResult>> {
        ...
        for result in &self.results {
            groups.entry(result.scenario_name.clone())
