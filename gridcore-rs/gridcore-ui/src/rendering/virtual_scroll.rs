...
impl VirtualScrollManager {
    ...
    pub fn get_visible_range(&self) -> (CellRange, CellRange) {
        ...
        let render_end_row =
            (start_row + self.viewport_rows + self.buffer_rows).min(self.total_rows);
        let render_end_col =
            (start_col + self.viewport_cols + self.buffer_cols).min(self.total_cols);
    ...
    pub fn is_visible(&self, row: u32, col: u32) -> bool {
        ...
        row >= visible.start_row
            && row < visible.end_row
            && col >= visible.start_col
            && col < visible.end_col
    }
