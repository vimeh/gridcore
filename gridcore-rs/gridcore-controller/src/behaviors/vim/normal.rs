...
use crate::state::{Action, UIState, InsertMode};
use super::{VimBehavior, Motion, Operator};

/// Handle normal mode key presses
impl VimBehavior {
    pub fn handle_normal_mode(&mut self, key: &str, current_state: &UIState) -> Result<Option<Action>> {
        ...
        match key {
            ...
            "R" => {
                ...
                Ok(Some(Action::EnterInsertMode { mode: Some(InsertMode::I) }))
            }
            ...
            "l" | "ArrowRight" => self.move_cursor(Motion::Right(count.unwrap_or(1)), current_state),
    ...
    fn handle_multi_char_command(&mut self, key: &str, count: Option<usize>, current_state: &UIState) -> Result<Option<Action>> {
        ...
        match command.as_str() {
            "g" => match key {
                ...
                "e" => self.move_cursor(Motion::WordEnd(count.unwrap_or(1).saturating_sub(1)), current_state),
                "E" => self.move_cursor(Motion::BigWordEnd(count.unwrap_or(1).saturating_sub(1)), current_state),
                _ => Ok(None),
            },
    ...
    fn move_cursor(&self, motion: Motion, current_state: &UIState) -> Result<Option<Action>> {
        ...
        let mut actions = vec![Action::UpdateCursor { cursor: new_position.clone() }];
        
        if let Some(new_viewport) = super::motion::adjust_viewport_for_motion(&new_position, current_state.viewport()) {
            actions.push(Action::UpdateViewport { viewport: new_viewport });
        }
    ...
    fn substitute_lines(&mut self, _count: usize) -> Result<Option<Action>> {
        self.mode = super::VimMode::Insert;
        Ok(Some(Action::EnterInsertMode { mode: Some(InsertMode::CapitalI) }))
    }
    ...
    fn jump_to_mark(&mut self, mark: char) -> Result<Option<Action>> {
        if let Some(address) = self.get_mark(mark) {
            Ok(Some(Action::UpdateCursor { cursor: address.clone() }))
        } else {
            Ok(None)
        }
    }
