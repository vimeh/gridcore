...
impl VimBehavior {
    ...
    fn process_normal_key(&mut self, key: &str, current_state: &UIState) -> Result<Option<Action>> {
        ...
        match key {
            // Mode changes
            "i" => Ok(Some(Action::EnterInsertMode { mode: Some(crate::state::InsertMode::I) })),
            "a" => Ok(Some(Action::EnterInsertMode { mode: Some(crate::state::InsertMode::A) })),
            "I" => Ok(Some(Action::EnterInsertMode { mode: Some(crate::state::InsertMode::CapitalI) })),
            "A" => Ok(Some(Action::EnterInsertMode { mode: Some(crate::state::InsertMode::CapitalA) })),
            "o" => Ok(Some(Action::EnterInsertMode { mode: Some(crate::state::InsertMode::O) })),
            "O" => Ok(Some(Action::EnterInsertMode { mode: Some(crate::state::InsertMode::CapitalO) })),
            ...
            // Movement
            "h" | "ArrowLeft" => self.create_movement_action(Motion::Left(count.unwrap_or(1)), current_state),
            "j" | "ArrowDown" => self.create_movement_action(Motion::Down(count.unwrap_or(1)), current_state),
            "k" | "ArrowUp" => self.create_movement_action(Motion::Up(count.unwrap_or(1)), current_state),
            "l" | "ArrowRight" => self.create_movement_action(Motion::Right(count.unwrap_or(1)), current_state),
            ...
            "w" => self.create_movement_action(Motion::WordForward(count.unwrap_or(1)), current_state),
            "b" => self.create_movement_action(Motion::WordBackward(count.unwrap_or(1)), current_state),
            "e" => self.create_movement_action(Motion::WordEnd(count.unwrap_or(1)), current_state),
            "W" => self.create_movement_action(Motion::BigWordForward(count.unwrap_or(1)), current_state),
            "B" => self.create_movement_action(Motion::BigWordBackward(count.unwrap_or(1)), current_state),
            "E" => self.create_movement_action(Motion::BigWordEnd(count.unwrap_or(1)), current_state),
    ...
    fn process_operator_pending_key(&mut self, key: &str, current_state: &UIState) -> Result<Option<Action>> {
        ...
        let motion = match key {
            ...
            "b" => Some(Motion::WordBackward(self.current_command.count.unwrap_or(1))),
    ...
    fn create_movement_action(&self, motion: Motion, current_state: &UIState) -> Result<Option<Action>> {
        let new_cursor = self.calculate_new_position(motion, current_state)?;
        Ok(Some(Action::UpdateCursor { cursor: new_cursor }))
    }
    
    fn calculate_new_position(&self, motion: Motion, current_state: &UIState) -> Result<CellAddress> {
        ...
        match motion {
            ...
            Motion::Right(n) => Ok(CellAddress::new(
                current.col + n as u32,
                current.row,
            )),
            ...
            Motion::Down(n) => Ok(CellAddress::new(
                current.col,
                current.row + n as u32,
            )),
