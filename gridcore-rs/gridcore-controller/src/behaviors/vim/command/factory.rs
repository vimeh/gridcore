...
impl CommandFactory {
    ...
    pub fn execute(&self, vim: &mut VimBehavior, parsed: &ExCommand) -> CommandResult {
        ...
        match category {
            Some(CommandCategory::FileOperations) => match parsed.command.as_str() {
            ...
            Some(CommandCategory::SearchReplace) => match parsed.command.as_str() {
            ...
            Some(CommandCategory::EditOperations) => match parsed.command.as_str() {
            ...
            Some(CommandCategory::CopyMove) => match parsed.command.as_str() {
                "t" | "copy" => vim.execute_copy(parsed),
                "m" | "move" => vim.execute_move(parsed),
                _ => Err(SpreadsheetError::InvalidCommand(format!(
                    "Unknown copy/move command: {}",
                    parsed.command
                ))),
            },
            Some(CommandCategory::Formatting) => match parsed.command.as_str() {
            ...
            Some(CommandCategory::Settings) => match parsed.command.as_str() {
            ...
            Some(CommandCategory::Marks) => match parsed.command.as_str() {
                "marks" => vim.execute_marks(parsed),
                "delmarks" => vim.execute_delmarks(parsed),
                _ => Err(SpreadsheetError::InvalidCommand(format!(
                    "Unknown marks command: {}",
                    parsed.command
                ))),
            },
            Some(CommandCategory::Registers) => match parsed.command.as_str() {
                "registers" => vim.execute_registers(parsed),
                "normal" => vim.execute_normal(parsed),
                _ => Err(SpreadsheetError::InvalidCommand(format!(
                    "Unknown register command: {}",
                    parsed.command
                ))),
            },
            ...
            None => Err(SpreadsheetError::InvalidCommand(format!(
                "Unknown command: {}",
                parsed.command
            ))),
        }
