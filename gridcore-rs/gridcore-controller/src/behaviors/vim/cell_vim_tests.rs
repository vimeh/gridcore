...
fn test_normal_mode_h_moves_left() {
    ...
    let action = vim.process_key("h").expect("Failed to process key 'h' in vim test");
...
fn test_normal_mode_l_moves_right() {
    ...
    let action = vim.process_key("l").expect("Failed to process key 'l' in vim test");
...
fn test_normal_mode_0_moves_to_start() {
    ...
    let action = vim.process_key("0").expect("Failed to process key '0' in vim test");
    assert_eq!(vim.get_cursor_position(), 0);
    assert!(matches!(action, Some(Action::UpdateEditingValue { .. })));
}
...
fn test_normal_mode_dollar_moves_to_end() {
    ...
    let action = vim.process_key("$").expect("Failed to process key '$' in vim test");
...
fn test_normal_mode_w_moves_word_forward() {
    ...
    vim.process_key("w").expect("Failed to process key 'w' in vim test");
...
fn test_normal_mode_b_moves_word_backward() {
    ...
    vim.process_key("b").expect("Failed to process key 'b' in vim test");
    assert_eq!(vim.get_cursor_position(), 6); // Start of "world"
...
fn test_normal_mode_e_moves_to_word_end() {
    ...
    vim.process_key("e").expect("Failed to process key 'e' in vim test");
...
fn test_i_enters_insert_mode_before_cursor() {
    ...
    let action = vim.process_key("i").expect("Failed to process key 'i' in vim test");
...
fn test_a_enters_insert_mode_after_cursor() {
    ...
    let action = vim.process_key("a").expect("Failed to process key 'a' in vim test");
    assert_eq!(vim.get_mode(), CellMode::Insert);
...
fn test_o_opens_line_below() {
    ...
    let action = vim.process_key("o").expect("Failed to process key 'o' in vim test");
    assert_eq!(vim.get_mode(), CellMode::Insert);
...
fn test_v_enters_visual_mode() {
    ...
    let action = vim.process_key("v").expect("Failed to process key 'v' in vim test");
...
fn test_visual_mode_h_extends_selection_left() {
    let mut vim = create_cell_vim("hello world");
    vim.cursor_position = 5;
    vim.process_key("v").expect("Failed to process key 'v' in vim test");

    vim.process_key("h").expect("Failed to process key 'h' in vim test");
    assert_eq!(vim.get_cursor_position(), 4);
...
fn test_visual_mode_l_extends_selection_right() {
    ...
    vim.process_key("l").expect("Failed to process key 'l' in vim test");
...
fn test_visual_mode_escape_exits() {
    ...
    let action = vim.process_key("Escape").expect("Failed to process key 'Escape' in vim test");
    assert_eq!(vim.get_mode(), CellMode::Normal);
...
fn test_visual_mode_d_deletes_selection() {
    ...
    vim.process_key("l").expect("Failed to process key 'l' in vim test");
    vim.process_key("l").expect("Failed to process key 'l' in vim test");
    vim.process_key("l").expect("Failed to process key 'l' in vim test"); // Select "hello"

    vim.process_key("d").expect("Failed to process key 'd' in vim test");
...
fn test_visual_mode_c_changes_selection() {
    ...
    let action = vim.process_key("c").expect("Failed to process key 'c' in vim test");
    assert_eq!(vim.get_text(), " world");
    assert_eq!(vim.get_mode(), CellMode::Insert);
    assert_eq!(vim.get_cursor_position(), 0);
...
fn test_visual_mode_y_yanks_selection() {
    ...
    let action = vim.process_key("y").expect("Failed to process key 'y' in vim test");
...
fn test_x_deletes_char() {
    ...
    vim.process_key("x").expect("Failed to process key 'x' in vim test");
...
fn test_dd_deletes_line() {
    ...
    vim.process_key("dd").expect("Failed to process key 'dd' in vim test");
    assert_eq!(vim.get_text(), "");
    assert_eq!(vim.get_cursor_position(), 0);
}
...
fn test_dw_deletes_word() {
    ...
    vim.process_key("dw").expect("Failed to process key 'dw' in vim test");
...
fn test_cc_changes_line() {
    ...
    let action = vim.process_key("cc").expect("Failed to process key 'cc' in vim test");
    assert_eq!(vim.get_text(), "");
    assert_eq!(vim.get_mode(), CellMode::Insert);
    assert!(matches!(action, Some(Action::EnterInsertMode { .. })));
}
...
fn test_cw_changes_word() {
    ...
    let action = vim.process_key("cw").expect("Failed to process key 'cw' in vim test");
    assert_eq!(vim.get_text(), "world");
    assert_eq!(vim.get_mode(), CellMode::Insert);
    assert_eq!(vim.get_cursor_position(), 0);
    assert!(matches!(action, Some(Action::EnterInsertMode { .. })));
...
fn test_s_substitutes_char() {
    ...
    let action = vim.process_key("s").expect("Failed to process key 's' in vim test");
    assert_eq!(vim.get_text(), "hllo");
    assert_eq!(vim.get_mode(), CellMode::Insert);
    assert_eq!(vim.get_cursor_position(), 1);
    assert!(matches!(action, Some(Action::EnterInsertMode { .. })));
...
fn test_insert_mode_typing() {
    let mut vim = create_cell_vim("hello");
    vim.process_key("i").expect("Failed to process key 'i' in vim test");
...
fn test_insert_mode_backspace() {
    ...
    vim.process_key("Backspace").expect("Failed to process key 'Backspace' in vim test");
...
fn test_insert_mode_delete() {
    ...
    vim.process_key("Delete").expect("Failed to process key 'Delete' in vim test");
    assert_eq!(vim.get_text(), "hllo");
    assert_eq!(vim.get_cursor_position(), 1);
}
...
fn test_yy_yanks_line() {
    ...
    vim.process_key("yy").expect("Failed to process key 'yy' in vim test");
...
fn test_yw_yanks_word() {
    ...
    vim.process_key("yw").expect("Failed to process key 'yw' in vim test");
    assert_eq!(vim.registers.get(&'"'), Some(&"hello ".to_string()));
    assert_eq!(vim.get_text(), "hello world"); // Text unchanged
}
...
fn test_p_pastes_after() {
    ...
    vim.process_key("p").expect("Failed to process key 'p' in vim test");
...
fn test_visual_mode_tilde_toggles_case() {
    ...
    vim.process_key("~").expect("Failed to process key '~' in vim test");
    assert_eq!(vim.get_text(), "hELlo");
    assert_eq!(vim.get_mode(), CellMode::Normal);
}
...
fn test_visual_mode_u_lowercases() {
    ...
    vim.process_key("u").expect("Failed to process key 'u' in vim test");
    assert_eq!(vim.get_text(), "helLO");
    assert_eq!(vim.get_mode(), CellMode::Normal);
}
