...
use crate::state::{Action, UIState, ResizeTarget};
...
impl ResizeBehavior {
    ...
    fn handle_navigation(&self, state: &UIState, direction: MoveDirection, is_horizontal: bool) -> Result<Option<Action>> {
    ...
    pub fn get_resize_info(&self, state: &UIState) -> Option<ResizeInfo> {
        if let UIState::Resize { target, current_size, original_size, .. } = state {
            Some(ResizeInfo {
                target: target.clone(),
