...
mod performance_tests {
    ...
    fn test_rapid_state_transitions() {
        ...
        assert!(transitions_per_second > 1000.0, 
                "Performance too low: {:.2} transitions/sec", transitions_per_second);
        
        println!("Handled {:.2} transitions per second", transitions_per_second);
    }
    ...
    fn test_large_selection_operations() {
        ...
        assert!(duration.as_millis() < 100, 
                "Large selection operations took too long: {:?}", duration);
    ...
    fn test_history_with_thousands_of_entries() {
        ...
        assert!(duration.as_secs() < 1, 
                "History operations took too long: {:?}", duration);
    ...
    fn test_event_listener_performance() {
        ...
        let mut machine = UIStateMachine::new(None);
        let counters: Vec<Arc<Mutex<u32>>> = (0..100)
        ...
        assert!(duration.as_millis() < 500, 
                "Event listener notifications took too long: {:?}", duration);
    ...
    fn test_complex_state_serialization_performance() {
        ...
        assert!(duration.as_millis() < 1000, 
                "State serialization too slow: {:?}", duration);
    ...
    fn test_viewport_scrolling_performance() {
        ...
        for i in 0..1000 {
            ...
            machine.transition(Action::UpdateViewport { viewport }).unwrap();
        ...
        assert!(updates_per_second > 5000.0, 
                "Viewport updates too slow: {:.2} updates/sec", updates_per_second);
        
        println!("Performed {:.2} viewport updates per second", updates_per_second);
    }
    ...
    fn test_command_parsing_performance() {
        ...
        assert!(updates_per_second > 10000.0, 
                "Command parsing too slow: {:.2} updates/sec", updates_per_second);
        
        println!("Processed {:.2} command character updates per second", updates_per_second);
    }
