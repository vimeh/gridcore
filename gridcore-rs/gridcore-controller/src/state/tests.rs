...
mod tests {
    ...
    fn test_navigation_to_editing() {
        ...
        match machine.get_state() {
            UIState::Editing { cell_mode, editing_value, cursor_position, .. } => {
    ...
    fn test_editing_mode_transitions() {
        ...
        match machine.get_state() {
            UIState::Editing { cell_mode, edit_variant, .. } => {
                assert_eq!(*cell_mode, CellMode::Insert);
    ...
    fn test_visual_mode_transitions() {
        ...
        match machine.get_state() {
            UIState::Editing { cell_mode, visual_type, visual_start, .. } => {
                assert_eq!(*cell_mode, CellMode::Normal);
    ...
    fn test_spreadsheet_visual_mode() {
        ...
        match machine.get_state() {
            UIState::Visual { visual_mode, selection: sel, .. } => {
    ...
    fn test_update_cursor_and_viewport() {
        ...
        machine.transition(Action::UpdateViewport {
    ...
    fn test_helper_methods() {
        ...
        match machine.get_state() {
            UIState::Editing { editing_value, cursor_position, .. } => {
                assert_eq!(editing_value, "test");
                assert_eq!(*cursor_position, 2);
            }
            _ => panic!("Expected editing state"),
        ...
        machine.enter_spreadsheet_visual_mode(
            SpreadsheetVisualMode::Line,
            selection,
        ).unwrap();
