...
mod refactoring_tests {
    ...
    fn test_core_state_shared_between_modes() {
        ...
        machine.transition(Action::UpdateCursor { cursor: new_cursor }).unwrap();
        ...
        machine.transition(Action::UpdateCursor { cursor: another_cursor }).unwrap();
    ...
    fn test_modal_consolidation_in_navigation() {
        ...
        let selection = Selection {
            selection_type: SelectionType::Cell { address: CellAddress::new(0, 0) },
            anchor: None,
        };
        ...
        // Test Insert modal
        machine.transition(Action::StartInsert {
        ...
        // Test Delete modal
        machine.transition(Action::StartDelete {
            targets: vec![1, 2, 3],
        ...
        machine.transition(Action::StartBulkOperation {
            parsed_command: ParsedBulkCommand::Format { format_type: "bold".to_string() },
    ...
    fn test_visual_selection_in_editing_state() {
        ...
        machine.transition(Action::EnterVisualMode {
            visual_type: VisualMode::Character,
            anchor: Some(2),
        }).unwrap();
        
        match machine.get_state() {
            UIState::Editing { mode, visual_selection, cursor_pos, .. } => {
        ...
        match machine.get_state() {
            UIState::Editing { mode, visual_selection, .. } => {
    ...
    fn test_consolidated_handler_coverage() {
        ...
        let valid_transitions = vec![
            (Action::UpdateCursor { cursor: CellAddress::new(1, 1) }, true),
            (Action::UpdateViewport { viewport: ViewportInfo { start_row: 0, start_col: 0, rows: 20, cols: 10 } }, true),
            (Action::EnterCommandMode, true),
            (Action::ExitCommandMode, true),
            (Action::StartEditing { edit_mode: None, initial_value: None, cursor_position: None }, true),
    ...
    fn test_invalid_action_rejection() {
        ...
        assert!(machine.transition(Action::EnterInsertMode { mode: Some(InsertMode::I) }).is_err());
        assert!(machine.transition(Action::ExitInsertMode).is_err());
        assert!(machine.transition(Action::UpdateEditingValue { value: "test".to_string(), cursor_position: 0 }).is_err());
        ...
        assert!(machine.transition(Action::StartBulkOperation {
            parsed_command: ParsedBulkCommand::Clear { clear_type: "all".to_string() },
            affected_cells: None,
        }).is_err());
    }
    ...
    fn test_modal_state_transitions() {
        ...
        machine.transition(Action::EnterCommandMode).unwrap();
        machine.transition(Action::UpdateCommandValue { value: ":w".to_string() }).unwrap();
        
        match machine.get_state() {
            UIState::Navigation { modal: Some(NavigationModal::Command { value }), .. } => {
                assert_eq!(value, ":w");
            }
    ...
    fn test_resize_modal_updates() {
        ...
        // Update resize
        machine.transition(Action::UpdateResize { delta: 50.0 }).unwrap();
    ...
    fn test_editing_mode_state_preservation() {
        ...
        match machine.get_state() {
            UIState::Editing { value, cursor_pos, mode, insert_variant, .. } => {
        ...
        // Update value
        machine.transition(Action::UpdateEditingValue {
            value: "test123".to_string(),
            cursor_position: 7,
        }).unwrap();
        
        match machine.get_state() {
            UIState::Editing { value, cursor_pos, .. } => {
