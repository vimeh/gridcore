...
mod complex_transition_tests {
    ...
    fn test_bulk_operation_workflow() {
        ...
        match machine.get_state() {
            UIState::BulkOperation { preview_available, .. } => {
                assert!(preview_available);
            }
            _ => panic!("Expected bulk operation state"),
        }
    ...
    fn test_resize_mode_workflow() {
        ...
        match machine.get_state() {
            UIState::Resize { target, current_position, .. } => {
        ...
        match machine.get_state() {
            UIState::Resize { current_position, .. } => {
    ...
    fn test_insert_delete_workflow() {
        ...
        // Update count
        machine.transition(Action::UpdateInsertCount { count: 5 }).unwrap();
        ...
        match machine.get_state() {
            UIState::Delete { targets, delete_type, .. } => {
    ...
    fn test_complex_visual_selection_expansion() {
        ...
        // Change to line mode
        machine.transition(Action::ChangeVisualMode {
        ...
        match machine.get_state() {
            UIState::Visual { visual_mode, selection, .. } => {
