...
impl WasmUIStateMachine {
    ...
    pub fn with_initial_state(state_js: JsValue) -> Result<WasmUIStateMachine, JsValue> {
        let state: UIState = from_value(state_js).map_err(|e| JsValue::from_str(&e.to_string()))?;
        Ok(Self {
    ...
    pub fn get_history(&self) -> Result<JsValue, JsValue> {
        to_value(&self.inner.get_history()).map_err(|e| JsValue::from_str(&e.to_string()))
    }
...
impl ActionBuilder {
    ...
    pub fn enter_command_mode() -> Result<JsValue, JsValue> {
        to_value(&Action::EnterCommandMode).map_err(|e| JsValue::from_str(&e.to_string()))
    }

    #[wasm_bindgen(js_name = "escape")]
    pub fn escape() -> Result<JsValue, JsValue> {
        to_value(&Action::Escape).map_err(|e| JsValue::from_str(&e.to_string()))
    }
