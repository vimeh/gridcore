...
mod tests {
    ...
    use crate::state::{UIState, Action, SpreadsheetMode, InsertMode};
    ...
    fn test_mode_transitions() {
        ...
        controller.handle_keyboard_event(event).unwrap();
        assert_eq!(controller.get_state().spreadsheet_mode(), SpreadsheetMode::Editing);
        ...
        controller.handle_keyboard_event(event).unwrap();
        assert_eq!(controller.get_state().spreadsheet_mode(), SpreadsheetMode::Command);
    }
    ...
    fn test_editing_mode_variants() {
        ...
        // Test 'i' for insert
        controller.handle_keyboard_event(KeyboardEvent::new("i".to_string())).unwrap();
        if let UIState::Editing { edit_variant, .. } = controller.get_state() {
            assert_eq!(*edit_variant, Some(InsertMode::I));
        }
        
        // Return to navigation
        controller.handle_keyboard_event(KeyboardEvent::new("Escape".to_string())).unwrap();
        
        // Test 'a' for append
        controller.handle_keyboard_event(KeyboardEvent::new("a".to_string())).unwrap();
        if let UIState::Editing { edit_variant, .. } = controller.get_state() {
    ...
    fn test_event_subscription() {
        ...
        controller.dispatch_action(Action::EnterCommandMode).unwrap();
    ...
    fn test_facade_integration() {
        ...
        controller.get_facade_mut().set_cell_value(&addr, "100").unwrap();
    ...
    fn test_command_mode_input() {
        ...
        // Enter command mode
        controller.handle_keyboard_event(KeyboardEvent::new(":".to_string())).unwrap();
        
        // Type command characters
        controller.handle_keyboard_event(KeyboardEvent::new("w".to_string())).unwrap();
        controller.handle_keyboard_event(KeyboardEvent::new("q".to_string())).unwrap();
    ...
    fn test_action_dispatch() {
        ...
        controller.dispatch_action(Action::UpdateCursor { cursor: new_cursor }).unwrap();
    ...
    fn test_complex_keyboard_sequence() {
        ...
        for _ in 0..5 {
            controller.handle_keyboard_event(KeyboardEvent::new("j".to_string())).unwrap();
        }
        ...
        for _ in 0..3 {
            controller.handle_keyboard_event(KeyboardEvent::new("l".to_string())).unwrap();
        }
    ...
    fn test_modifiers_handling() {
        ...
        let event = KeyboardEvent::new("s".to_string())
            .with_modifiers(false, true, false, false); // Ctrl+S
