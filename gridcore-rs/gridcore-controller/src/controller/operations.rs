...
impl<'a> CellOperations<'a> {
    ...
    pub fn set_cursor(&mut self, address: CellAddress) -> Result<()> {
        self.controller.dispatch_action(Action::UpdateCursor { cursor: address })
    }
}
...
impl<'a> ErrorOperations<'a> {
    ...
    pub fn emit(&mut self, message: String, severity: ErrorSeverity) {
        ...
        self.controller.get_error_manager_mut().add_error(message.clone(), severity);
        
        // Dispatch event for UI updates
        self.controller.dispatch_event(crate::controller::events::SpreadsheetEvent::ErrorOccurred { message, severity });
    }
        ...
        if let Some(sel) = selection {
            ...
            match &sel.selection_type {
                SelectionType::Range { start, end } => {
                    crate::behaviors::selection_stats::calculate_range(self.controller.facade(), start, end)
                }
                ...
                SelectionType::Cell { address } => {
                    crate::behaviors::selection_stats::calculate_single_cell(self.controller.facade(), address)
                }
                SelectionType::Multi { selections } => {
                    // For multi-selection, calculate stats for all ranges
                    let ranges: Vec<_> = selections.iter().filter_map(|s| {
                    ...
                    crate::behaviors::selection_stats::calculate_multi_range(self.controller.facade(), &ranges)
                }
            }
        } else {
