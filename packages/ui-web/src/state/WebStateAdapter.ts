...
import type { UIState, ViewportInfo } from "@gridcore/ui-core"
import type { Result } from "@gridcore/ui-core"
import type { Action } from "@gridcore/ui-core"
...
export class WebStateAdapter {
  ...
  toggleInteractionMode(): void {
    this.interactionMode = this.interactionMode === "normal" ? "keyboard-only" : "normal"
    this.notifyListeners()
  }
  ...
  private navigateToCell(deltaRow: number, deltaCol: number): Result<UIState> {
    let result: Result<UIState> = { ok: true, value: this.controller.getState() }
