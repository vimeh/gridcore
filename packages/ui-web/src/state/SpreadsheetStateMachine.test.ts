...
describe("SpreadsheetStateMachine", () => {
  ...
  describe("navigation state transitions", () => {
    ...
    test("can start editing with initial mode", () => {
      const result = stateMachine.transition({ type: "START_EDITING", editMode: "append" })
      expect(result.ok).toBe(true)
    ...
    test("can toggle interaction mode", () => {
      const result = stateMachine.transition({ type: "TOGGLE_INTERACTION_MODE" })
      expect(result.ok).toBe(true)
      expect(stateMachine.isInKeyboardOnlyMode()).toBe(true)
    })

    test("can set interaction mode", () => {
      const result = stateMachine.transition({ type: "SET_INTERACTION_MODE", mode: "keyboard-only" })
      expect(result.ok).toBe(true)
  ...
  describe("editing state transitions", () => {
    ...
    test("preserves interaction mode when stopping editing", () => {
      stateMachine.transition({ type: "SET_INTERACTION_MODE", mode: "keyboard-only" })
  ...
  describe("normal editing substate transitions", () => {
    ...
    test("can enter insert mode with specific mode", () => {
      const result = stateMachine.transition({ type: "ENTER_INSERT_MODE", mode: "append" })
      expect(result.ok).toBe(true)
      expect(stateMachine.getInsertMode()).toBe("append")
    })

    test("can enter visual mode", () => {
      const result = stateMachine.transition({ type: "ENTER_VISUAL_MODE", visualType: "character" })
      expect(result.ok).toBe(true)
    ...
    test("can enter visual line mode", () => {
      const result = stateMachine.transition({ type: "ENTER_VISUAL_MODE", visualType: "line" })
      expect(result.ok).toBe(true)
      expect(stateMachine.getVisualMode()).toBe("line")
    })

    test("can enter visual block mode", () => {
      const result = stateMachine.transition({ type: "ENTER_VISUAL_BLOCK_MODE" })
      expect(result.ok).toBe(true)
  ...
  describe("insert mode transitions", () => {
    ...
    test("can change edit mode", () => {
      const result = stateMachine.transition({ type: "SET_EDIT_MODE", mode: "replace" })
      expect(result.ok).toBe(true)
  ...
  describe("visual mode transitions", () => {
    beforeEach(() => {
      stateMachine.transition({ type: "START_EDITING" })
      stateMachine.transition({ type: "ENTER_VISUAL_MODE", visualType: "character" })
    })
  ...
  describe("query methods", () => {
    test("mode string representation", () => {
      ...
      stateMachine.transition({ type: "ENTER_VISUAL_MODE", visualType: "line" })
