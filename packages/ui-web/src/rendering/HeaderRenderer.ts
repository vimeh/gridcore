...
export class HeaderRenderer {
  ...
  private setupCanvases(): void {
    ...
    [
      { canvas: this.rowHeaderCanvas, ctx: this.rowHeaderCtx, name: 'rowHeader' },
      { canvas: this.colHeaderCanvas, ctx: this.colHeaderCtx, name: 'colHeader' },
      { canvas: this.cornerCanvas, ctx: this.cornerCtx, name: 'corner' }
    ].forEach(({ canvas, ctx, name }) => {
      const rect = canvas.getBoundingClientRect();
      if (rect.width === 0 || rect.height === 0) {
        console.warn(`HeaderRenderer: ${name} canvas has zero dimensions`, rect);
      }
