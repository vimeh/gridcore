...
export class EvaluatorWrapper {
  ...
  static checkCircularDependencies(
    ...
    return wasm.checkCircularDependencies(formula, currentCell, dependencyGraph)
  }
...
export class WasmEvaluator extends EvaluatorWrapper {
  constructor() {
    console.warn("WasmEvaluator is deprecated, use EvaluatorWrapper static methods instead")
  }
  ...
  checkCircular(formula: string, currentCell: string, getDependencies: Function): boolean {
    // For backward compatibility
    const dependencyGraph: Record<string, string[]> = {}
    return EvaluatorWrapper.checkCircularDependencies(formula, currentCell, dependencyGraph)
  }
}