...
describe("StateAdapter", () => {
  ...
  test("should convert column indices to letters", () => {
    const state0 = createNavigationState({ row: 0, col: 0 } as CellAddress, mockViewport)
    const state25 = createNavigationState({ row: 0, col: 25 } as CellAddress, mockViewport)
    const state26 = createNavigationState({ row: 0, col: 26 } as CellAddress, mockViewport)
    const state52 = createNavigationState({ row: 0, col: 52 } as CellAddress, mockViewport)
  ...
  test("should format resize mode display", () => {
    ...
    expect(StateAdapter.getResizeModeDisplay(increaseInfo)).toBe("COLUMN 5: 20 (+10)")
    ...
    expect(StateAdapter.getResizeModeDisplay(decreaseInfo)).toBe("ROW 3: 5 (-3)")
  })
