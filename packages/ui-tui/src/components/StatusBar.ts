...
export class StatusBarComponent extends Renderable {
  ...
  protected renderSelf(buffer: OptimizedBuffer): void {
    ...
    if (mode === 'command' && commandValue !== undefined) {
      ...
      currentX += 1
      buffer.setText(currentX, pos.y, commandValue + '█', this.colors.command, this.colors.bg)
    ...
    if (shortcutsX > currentX + 2) {
      buffer.setText(shortcutsX, pos.y, shortcuts, this.colors.fg, this.colors.bg)
    }
  }
  ...
  private getGridInfo(): string {
    ...
    if (selectedRange) {
      const rows = Math.abs(selectedRange.end.row - selectedRange.start.row) + 1
      const cols = Math.abs(selectedRange.end.col - selectedRange.start.col) + 1
