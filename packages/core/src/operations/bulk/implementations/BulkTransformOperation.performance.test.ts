...
import { BulkTransformOperation, BulkTransformOptions, TransformationType } from "./BulkTransformOperation";
...
function generateTestData(repository: MockCellRepository, count: number, dataType: 'text' | 'mixed' = 'text') {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (dataType === 'mixed') {
      ...
      switch (i % 4) {
        case 0: value = `text value ${i}`; break;
        case 1: value = i * 3.14; break;
        case 2: value = i % 2 === 0; break;
        case 3: value = `  spaced text ${i}  `; break;
      }
    } else {
      // Pure text data with various cases
      switch (i % 5) {
        case 0: value = `lowercase text ${i}`; break;
        case 1: value = `UPPERCASE TEXT ${i}`; break;
        case 2: value = `  Trimmed Text ${i}  `; break;
        case 3: value = `Mixed\nCase\tText\r${i}`; break;
        case 4: value = `multiple   spaces   text   ${i}`; break;
      }
    }
...
describe("BulkTransformOperation Performance Tests", () => {
  ...
  describe("Uppercase Transformation Performance", () => {
    it("should process 10k cells in under 1 second", async () => {
      ...
      console.log(`Uppercase 10k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should process 50k cells with excellent performance", async () => {
      ...
      console.log(`Uppercase 50k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should handle 100k cells efficiently", async () => {
      ...
      console.log(`Uppercase 100k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Lowercase Transformation Performance", () => {
    it("should process 25k cells with high performance", async () => {
      ...
      console.log(`Lowercase 25k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Clean Transformation Performance", () => {
    ...
    it("should handle complex clean operations efficiently", async () => {
      ...
      console.log(`Complex clean 15k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Mixed Data Type Performance", () => {
    it("should handle mixed data types efficiently", async () => {
      ...
      console.log(`Mixed data 40k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should handle type conversion efficiently", async () => {
      ...
      console.log(`Type conversion 25k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Memory Efficiency", () => {
    it("should maintain reasonable memory usage for large operations", async () => {
      ...
      console.log(`Memory usage 75k cells: ${Math.round(memDelta / 1024 / 1024)}MB total, ${Math.round(memPerCell)}B per cell`);
    });
  });
  ...
  describe("Batch Size Optimization", () => {
    it("should find optimal batch size for performance", async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        const operation = new BulkTransformOperation(selection, options, freshRepo);
  ...
  describe("Stress Testing", () => {
    it("should handle maximum size selection", async () => {
      ...
      console.log(`Stress test 250k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
