...
import { BulkFormatOperation, BulkFormatOptions, FormatType } from "./BulkFormatOperation";
...
function generateTestData(repository: MockCellRepository, count: number, dataType: 'numeric' | 'mixed' | 'dates' = 'numeric') {
  ...
  for (let i = 0; i < count; i++) {
    ...
    if (dataType === 'mixed') {
      ...
      switch (i % 6) {
        case 0: value = (i + 1) * 123.45; break;
        case 1: value = (i * 0.01); break; // Percentages
        case 2: value = `${(i + 1) * 100}.50`; break; // String numbers
        case 3: value = new Date(2024, i % 12, (i % 28) + 1); break;
        case 4: value = i % 2 === 0; break;
        case 5: value = `text value ${i}`; break;
      }
    } else if (dataType === 'dates') {
      // Pure date data
      switch (i % 4) {
        case 0: value = new Date(2024, i % 12, (i % 28) + 1); break;
        case 1: value = new Date(2024, i % 12, (i % 28) + 1, 14, 30, 0); break;
        case 2: value = `2024-${String((i % 12) + 1).padStart(2, '0')}-${String((i % 28) + 1).padStart(2, '0')}`; break;
        case 3: value = 44927 + i; break; // Excel date serial numbers
      }
    } else {
      // Pure numeric data
      switch (i % 5) {
        case 0: value = (i + 1) * 1234.56; break;
        case 1: value = (i + 1) * 0.1234; break; // Decimal percentages
        case 2: value = Math.floor((i + 1) * 100); break; // Integers
        case 3: value = -((i + 1) * 567.89); break; // Negative numbers
        case 4: value = (i + 1) / 10000; break; // Small decimals
      }
    }
...
function measurePerformance<T>(operation: () => Promise<T>): Promise<{ result: T; timeMs: number; cellsPerSecond: number; cellCount: number }> {
  return new Promise(async (resolve) => {
    ...
    let cellCount = 0;
    if (typeof result === 'object' && result !== null && 'cellsProcessed' in result) {
      cellCount = (result as any).cellsProcessed;
    }
    
    const cellsPerSecond = cellCount > 0 ? Math.round(cellCount / (timeMs / 1000)) : 0;
...
describe("BulkFormatOperation Performance Tests", () => {
  ...
  describe("Currency Formatting Performance", () => {
    it("should process 10k cells in under 1 second", async () => {
      ...
      const { result, timeMs, cellsPerSecond } = await measurePerformance(() => operation.execute());
      
      expect(result.success).toBe(true);
      expect(timeMs).toBeLessThan(1000);
      expect(cellsPerSecond).toBeGreaterThan(10000); // Target: > 10k cells/second
      
      console.log(`Currency 10k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should process 50k cells with excellent performance", async () => {
      ...
      console.log(`Currency 50k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should handle 100k cells efficiently", async () => {
      ...
      console.log(`Currency 100k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Percentage Formatting Performance", () => {
    it("should process 25k cells with high performance", async () => {
      ...
      console.log(`Percent 25k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should handle complex percentage formatting", async () => {
      ...
      console.log(`Complex percent 40k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Number Formatting Performance", () => {
    ...
    it("should handle advanced number formatting", async () => {
      ...
      console.log(`Advanced number 30k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Date Formatting Performance", () => {
    ...
    it("should handle complex date formatting with time", async () => {
      ...
      console.log(`Complex date 12k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should handle locale-specific date formatting", async () => {
      ...
      console.log(`Locale date 20k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Mixed Data Type Performance", () => {
    it("should handle mixed data types efficiently", async () => {
      ...
      console.log(`Mixed data 45k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should handle type conversion efficiently", async () => {
      ...
      console.log(`Type conversion 30k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
  });
  ...
  describe("Locale Performance", () => {
    it("should maintain performance with different locales", async () => {
      ...
      const results: { locale: string; timeMs: number; cellsPerSecond: number }[] = [];
      ...
      for (const locale of locales) {
        ...
        const operation = new BulkFormatOperation(selection, options, freshRepo);
        
        const { timeMs, cellsPerSecond } = await measurePerformance(() => operation.execute());
        results.push({ locale, timeMs, cellsPerSecond });
        
        expect(cellsPerSecond).toBeGreaterThan(8000); // Should maintain good performance
        console.log(`Locale ${locale}: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
      }
      
      // Performance should be consistent across locales
      const avgPerformance = results.reduce((sum, r) => sum + r.cellsPerSecond, 0) / results.length;
  ...
  describe("Memory Efficiency", () => {
    it("should maintain reasonable memory usage for large operations", async () => {
      ...
      const { result, timeMs } = await measurePerformance(() => operation.execute());
      ...
      console.log(`Memory usage 60k cells: ${Math.round(memDelta / 1024 / 1024)}MB total, ${Math.round(memPerCell)}B per cell`);
    });
  });
  ...
  describe("Batch Size Optimization", () => {
    it("should find optimal batch size for currency formatting", async () => {
      ...
      const results: { batchSize: number; timeMs: number; cellsPerSecond: number }[] = [];
      ...
      for (const batchSize of batchSizes) {
        ...
        console.log(`Batch size ${batchSize}: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
      }
      ...
      console.log(`Best batch size: ${bestResult.batchSize} (${bestResult.cellsPerSecond} cells/sec)`);
    });
  });
  ...
  describe("Stress Testing", () => {
    it("should handle maximum size selection", async () => {
      ...
      console.log(`Stress test 200k cells: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
    });
    ...
    it("should maintain performance under concurrent operations", async () => {
      ...
      for (let i = 0; i < results.length; i++) {
        ...
        console.log(`Concurrent operation ${i + 1}: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
      }
    });
  });
  ...
  describe("Format-Specific Performance", () => {
    it("should compare performance across different format types", async () => {
      ...
      const formatTypes: FormatType[] = ["text", "number", "currency", "percent"];
      const results: { formatType: string; timeMs: number; cellsPerSecond: number }[] = [];
      ...
      for (const formatType of formatTypes) {
        ...
        console.log(`Format ${formatType}: ${timeMs}ms, ${cellsPerSecond} cells/sec`);
      }
