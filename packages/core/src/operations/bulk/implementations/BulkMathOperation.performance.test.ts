...
import { BulkMathOperation, BulkMathOptions } from "./BulkMathOperation";
...
class PerformanceMockCellRepository implements ICellRepository {
  ...
  initializeCells(cellCount: number): void {
    ...
    for (let i = 0; i < cellCount; i++) {
      ...
      const type = i % 4;
      switch (type) {
        case 0: value = Math.random() * 1000; break;           // Numbers
        case 1: value = (Math.random() * 1000).toString(); break; // Numeric strings  
        case 2: value = `$${Math.random() * 1000}`; break;     // Formatted strings
        case 3: value = Math.random() > 0.9 ? "text" : Math.random() * 100; break; // Mostly numeric
      }
...
describe("BulkMathOperation Performance Tests", () => {
  ...
  describe("Scale Performance Tests", () => {
    it("should handle 1,000 cells efficiently", async () => {
      ...
      const cellsPerSecond = await measurePerformance(operation, cellCount, "1K cells - Add operation");
    ...
    it("should handle 10,000 cells efficiently", async () => {
      ...
      const operation = new BulkMathOperation(selection, options, cellRepository);
      const cellsPerSecond = await measurePerformance(operation, cellCount, "10K cells - Multiply operation");
    ...
    it("should handle 50,000 cells efficiently", async () => {
      ...
      const operation = new BulkMathOperation(selection, options, cellRepository);
      const cellsPerSecond = await measurePerformance(operation, cellCount, "50K cells - Divide operation");
    ...
    it("should handle 100,000 cells efficiently", async () => {
      ...
      const operation = new BulkMathOperation(selection, options, cellRepository);
      const cellsPerSecond = await measurePerformance(operation, cellCount, "100K cells - Add operation (target test)");
  ...
  describe("Operation Type Performance Comparison", () => {
    ...
    it("should compare basic arithmetic operations performance", async () => {
      ...
      results.add = await measurePerformance(addOp, cellCount, "25K cells - Addition");

      // Test subtraction  
      const subOp = new BulkMathOperation(selection, { operation: "subtract", value: 5 }, cellRepository);
      results.subtract = await measurePerformance(subOp, cellCount, "25K cells - Subtraction");

      // Test multiplication
      const mulOp = new BulkMathOperation(selection, { operation: "multiply", value: 2 }, cellRepository);
      results.multiply = await measurePerformance(mulOp, cellCount, "25K cells - Multiplication");

      // Test division
      const divOp = new BulkMathOperation(selection, { operation: "divide", value: 3 }, cellRepository);
      results.divide = await measurePerformance(divOp, cellCount, "25K cells - Division");
    ...
    it("should compare complex operations performance", async () => {
      ...
      // Test modulo
      const modOp = new BulkMathOperation(selection, { operation: "modulo", value: 7 }, cellRepository);
      results.modulo = await measurePerformance(modOp, cellCount, "25K cells - Modulo");

      // Test percentage increase
      const percentOp = new BulkMathOperation(selection, { operation: "percent", value: 25 }, cellRepository);
      results.percent = await measurePerformance(percentOp, cellCount, "25K cells - Percentage");

      // Test rounding
      const roundOp = new BulkMathOperation(selection, { operation: "round", value: 0, decimalPlaces: 2 }, cellRepository);
      results.round = await measurePerformance(roundOp, cellCount, "25K cells - Rounding");
  ...
  describe("Configuration Impact on Performance", () => {
    ...
    it("should measure impact of string conversion", async () => {
      ...
      const withConversion = new BulkMathOperation(selection, {
        operation: "add",
        value: 10,
        convertStrings: true
      }, cellRepository);
      const withConversionPerf = await measurePerformance(withConversion, cellCount, "20K cells - With string conversion");

      // Test with string conversion disabled
      cellRepository.initializeCells(cellCount); // Reset
      const withoutConversion = new BulkMathOperation(selection, {
        operation: "add", 
        value: 10,
        convertStrings: false
      }, cellRepository);
      const withoutConversionPerf = await measurePerformance(withoutConversion, cellCount, "20K cells - Without string conversion");
      ...
      console.log(`String conversion impact: ${((withConversionPerf / withoutConversionPerf - 1) * 100).toFixed(1)}%`);
    });
    ...
    it("should measure impact of type preservation", async () => {
      ...
      const withPreservation = new BulkMathOperation(selection, {
        operation: "multiply",
        value: 2,
        preserveType: true
      }, cellRepository);
      const withPreservationPerf = await measurePerformance(withPreservation, cellCount, "20K cells - With type preservation");

      // Test without type preservation
      cellRepository.initializeCells(cellCount); // Reset
      const withoutPreservation = new BulkMathOperation(selection, {
        operation: "multiply",
        value: 2,
        preserveType: false
      }, cellRepository);
      const withoutPreservationPerf = await measurePerformance(withoutPreservation, cellCount, "20K cells - Without type preservation");
      ...
      console.log(`Type preservation impact: ${((withPreservationPerf / withoutPreservationPerf - 1) * 100).toFixed(1)}%`);
    });
    ...
    it("should measure impact of batch size", async () => {
      ...
      for (const batchSize of batchSizes) {
        ...
        const operation = new BulkMathOperation(selection, {
          operation: "add",
          value: 7,
          batchSize
        }, cellRepository);
      ...
      Object.entries(results).forEach(([config, perf]) => {
        console.log(`  ${config}: ${Math.round(perf).toLocaleString()} cells/sec`);
      });
    });
  });
  ...
  describe("Memory Usage Estimation", () => {
    it("should provide reasonable memory estimates", () => {
      ...
      cellCounts.forEach(cellCount => {
        const selection = createLargeSelection(cellCount);
        const operation = new BulkMathOperation(selection, { operation: "add", value: 1 }, cellRepository);
        
        // Call the protected method through any to test it
        const memoryEstimate = (operation as any).estimateMemoryUsage(cellCount);
        
        console.log(`${cellCount.toLocaleString()} cells - Memory estimate: ${(memoryEstimate / 1024).toFixed(1)}KB`);
  ...
  describe("Stress Tests", () => {
    it("should handle very large selections without crashing", async () => {
      ...
      const cellsPerSecond = await measurePerformance(operation, cellCount, "250K cells - Stress test");
    ...
    it("should maintain performance consistency across multiple operations", async () => {
      ...
      for (let i = 0; i < iterations; i++) {
        ...
        console.log(`  Iteration ${i + 1}: ${Math.round(cellsPerSecond).toLocaleString()} cells/sec`);
        expect(result.success).toBe(true);
      }
      ...
      const variationPercent = ((maxPerformance - minPerformance) / avgPerformance) * 100;

      console.log(`  Average: ${Math.round(avgPerformance).toLocaleString()} cells/sec`);
