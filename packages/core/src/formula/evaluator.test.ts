...
describe("FormulaEvaluator", () => {
  ...
  describe("cell references", () => {
    test("evaluates cell reference", () => {
      const cells = new Map([["0,0", { rawValue: 42, computedValue: 42 }]]);
    ...
    test("handles empty cells as zero", () => {
      const cells = new Map([["0,0", { rawValue: 10, computedValue: 10 }]]);
      const result = evaluator.evaluate("=A1+B1", createContext(cells));
  ...
  describe("functions", () => {
    ...
    describe("logical functions", () => {
      test("AND function", () => {
        expect(
          evaluator.evaluate("=AND(TRUE,TRUE)", createContext(new Map())).value,
        ).toBe(true);
        expect(
          evaluator.evaluate("=AND(TRUE,FALSE)", createContext(new Map()))
            .value,
        ).toBe(false);
        expect(
          evaluator.evaluate("=AND(5>3,10>5)", createContext(new Map())).value,
        ).toBe(true);
      });

      test("OR function", () => {
        expect(
          evaluator.evaluate("=OR(TRUE,FALSE)", createContext(new Map())).value,
        ).toBe(true);
        expect(
          evaluator.evaluate("=OR(FALSE,FALSE)", createContext(new Map()))
            .value,
        ).toBe(false);
      });
      ...
      test("NOT function", () => {
        expect(
          evaluator.evaluate("=NOT(TRUE)", createContext(new Map())).value,
        ).toBe(false);
        expect(
          evaluator.evaluate("=NOT(FALSE)", createContext(new Map())).value,
        ).toBe(true);
      });
    });
