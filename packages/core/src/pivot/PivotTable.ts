...
import type { PivotTableConfig, PivotTableOutput, PivotTableMetadata } from "./PivotTypes"
...
export class PivotTable {
  ...
  generate(sourceGrid: Grid, outputTopLeft: CellAddress): PivotTableOutput {
    ...
    const sortedRowKeys = Array.from(uniqueRowKeys).filter(key => key !== undefined).sort()
    const sortedColumnKeys = Array.from(uniqueColumnKeys).filter(key => key !== undefined).sort()
    ...
    const headerRows = columnFieldCount === 0 ? 1 : columnFieldCount + (valueFieldCount > 1 ? 1 : 0)
    ...
    if (this.config.columnFields.length === 0) {
      ...
      this.config.valueFields.forEach((valueField, vfIndex) => {
        ...
        const label = valueField.alias || `${valueField.fieldName} (${valueField.aggregator})`
      ...
      sortedColumnKeys.forEach((_, colIndex) => {
        this.config.valueFields.forEach((valueField, vfIndex) => {
          ...
          const col = startCol + headerCols + colIndex * valueFieldCount + vfIndex
          const label = valueField.alias || `${valueField.fieldName} (${valueField.aggregator})`
          output.set(`${row},${col}`, label)
        })
      })
    ...
    sortedRowKeys.forEach((rowKey, rowIndex) => {
      ...
      if (rowData) {
        if (this.config.columnFields.length === 0) {
          ...
          if (cellData) {
            this.config.valueFields.forEach((valueField, vfIndex) => {
              const fieldAlias = valueField.alias || `${valueField.fieldName} (${valueField.aggregator})`
    ...
    if (this.config.showRowTotals || this.config.showColumnTotals || this.config.showGrandTotals) {
      this.addTotals(output, sortedRowKeys, sortedColumnKeys, aggregatedData, outputTopLeft, headerRows, headerCols, valueFieldCount)
    }

    // Calculate dimensions
    const totalRows = headerRows + sortedRowKeys.length + (this.config.showColumnTotals ? 1 : 0)
    const totalCols = headerCols + sortedColumnKeys.length * valueFieldCount + (this.config.showRowTotals ? valueFieldCount : 0)
  ...
  private addTotals(
    ...
    if (this.config.showRowTotals) {
      const totalColStart = startCol + headerCols + sortedColumnKeys.length * valueFieldCount
    ...
    // Grand total
    if (this.config.showGrandTotals && this.config.showRowTotals && this.config.showColumnTotals) {
      const totalRowStart = startRow + headerRows + sortedRowKeys.length
      const totalColStart = startCol + headerCols + sortedColumnKeys.length * valueFieldCount
