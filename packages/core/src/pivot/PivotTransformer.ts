import type { Cell, CellRange, CellValueType } from "../types"
...
export class PivotTransformer {
  ...
  private loadSourceData(): void {
    ...
    for (let col = range.start.col; col <= range.end.col; col++) {
      ...
      const header = String(cell?.computedValue ?? cell?.rawValue ?? `Column${col}`)
      headers.push(header)
  ...
  private applyFilters(data: Array<Record<string, CellValueType>>): Array<Record<string, CellValueType>> {
  ...
  private aggregateData(
    ...
    for (const record of data) {
      const rowKey = this.config.rowFields.length > 0
        ? this.config.rowFields
            .map(field => String(record[field] ?? ""))
            .join("|")
        : ""
      const columnKey = this.config.columnFields.length > 0
    ...
    for (const [key, values] of groupedData) {
      ...
      const valueField = this.config.valueFields.find(vf => vf.fieldName === fieldName)
      if (valueField) {
