...
import type { WorkbookState, SheetState } from "./types/WorkbookState"
...
describe("WorkbookState", () => {
  ...
  describe("Workbook state management", () => {
    ...
    test("restores workbook from state", () => {
      ...
      // Verify data
      expect(restoredSheet2?.getEngine().getCell(address)?.rawValue).toBe("Test Data")
  ...
  describe("State round-trip integrity", () => {
    test("maintains data integrity through state conversion", () => {
      ...
      expect(restoredSheet2.getEngine().getCell({ row: 0, col: 0 })?.rawValue).toBe("Text")
      expect(restoredSheet2.getEngine().getCell({ row: 1, col: 1 })?.rawValue).toBe(123.45)
      expect(restoredSheet3.getEngine().getCell({ row: 2, col: 2 })?.rawValue).toBe(true)
      expect(restoredSheet4.getEngine().getCell({ row: 3, col: 3 })?.formula).toBe("=A1+B2")
