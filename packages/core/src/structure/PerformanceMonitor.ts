...
export class PerformanceMonitor {
   ...
   */
  startOperation(operationName: string, operationSize: number): PerformanceTimer {
  ...
  getPerformanceReport(): PerformanceReport {
    ...
    const avgDurationScore = averageDuration / this.thresholds.warningDurationMs;
    const memoryScore = peakMemoryUsage / (this.thresholds.warningMemoryMB * 1024 * 1024);
    const score = (violationRate * 0.4) + (avgDurationScore * 0.4) + (memoryScore * 0.2);
  ...
  getPerformanceWarnings(): string[] {
    ...
    if (report.operationsExceedingThreshold > 0) {
      warnings.push(`${report.operationsExceedingThreshold} operations exceeded performance thresholds`);
    }

    if (report.peakMemoryUsage > this.thresholds.warningMemoryMB * 1024 * 1024) {
      warnings.push(`Peak memory usage ${Math.round(report.peakMemoryUsage / 1024 / 1024)}MB exceeds warning threshold`);
    }
  ...
  private checkPerformanceThresholds(metrics: PerformanceMetrics): void {
    ...
    if (metrics.duration > expectedDuration) {
      console.warn(`Performance warning: ${metrics.operationName} took ${metrics.duration}ms, expected ${expectedDuration}ms for ${metrics.operationSize} rows/cols`);
    }

    const memoryUsageMB = metrics.memoryAfter / 1024 / 1024;
    if (memoryUsageMB > this.thresholds.warningMemoryMB) {
      console.warn(`Memory warning: ${metrics.operationName} used ${memoryUsageMB.toFixed(2)}MB`);
    }
  }
  ...
  private calculatePerformanceGrade(): 'A' | 'B' | 'C' | 'D' | 'F' {
    ...
    const report = this.getPerformanceReport();
    const violationRate = report.operationsExceedingThreshold / report.totalOperations;
    const avgDurationScore = report.averageDuration / this.thresholds.warningDurationMs;
    const memoryScore = report.peakMemoryUsage / (this.thresholds.warningMemoryMB * 1024 * 1024);
...
export class PerformanceTimer {
  ...
  constructor(monitor: PerformanceMonitor, operationName: string, operationSize: number) {
