...
export class CalculationService implements ICalculationService {
  ...
  calculateCell(address: CellAddress): Result<Cell> {
    ...
    try {
      ...
      const evalResult = this.formulaService.evaluateFormula(cell.formula!, context)
  ...
  calculateRange(addresses: CellAddress[]): Result<Map<string, Cell>> {
    ...
    for (const address of addresses) {
      ...
      if (!result.ok) {
        return err(`Failed to calculate ${address.toString()}: ${result.error}`)
      }
      
      // Check if the cell has an error
      if (result.value.hasError()) {
        return err(`Cell ${address.toString()} has error: ${result.value.error}`)
      }
