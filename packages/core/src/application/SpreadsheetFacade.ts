...
export class SpreadsheetFacade implements ISpreadsheetFacade {
  ...
  // biome-ignore lint/correctness/noUnusedPrivateClassMembers: this IS used later on in this very file
    ...
    try {
      ...
      return err(
        error instanceof Error
          ? error.message
          : "Unknown error setting cell value",
      );
    }
  }

  private performSetCell(
    address: CellAddress,
    newCell: Cell,
    ...
    try {
      ...
      for (const [key, cell] of cells) {
        if (cell.hasFormula()) {
          ...
          if (addr.ok) {
            const calcResult = this.calculationService.calculateCell(
              addr.value,
            );
            if (calcResult.ok) {
      ...
      return err(
        error instanceof Error
          ? error.message
          : "Unknown error getting cells in range",
      );
    }
  }
  ...
  setCellsInRange(
    ...
    try {
      ...
      return err(
        error instanceof Error
          ? error.message
          : "Unknown error setting cells in range",
      );
    }
  }
  ...
  deleteCellsInRange(range: CellRange): Result<void> {
    ...
    try {
      ...
      return err(
        error instanceof Error
          ? error.message
          : "Unknown error deleting cells in range",
      );
    }
  }
  ...
  recalculate(): Result<void> {
    try {
      ...
      return err(
        error instanceof Error
          ? error.message
          : "Unknown error during recalculation",
      );
    }
  }
  ...
  commitBatch(batchId: string): Result<void> {
    ...
    try {
      ...
      const calcResult =
        this.calculationService.calculateRange(affectedAddresses);
      if (!calcResult.ok) {
        return err(calcResult.error);
      }
      ...
      return err(
        error instanceof Error
          ? error.message
          : "Unknown error committing batch",
      );
    }
  }
