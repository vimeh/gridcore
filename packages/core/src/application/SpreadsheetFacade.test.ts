...
import type { CellValueChangedEvent, BatchUpdateStartedEvent, BatchUpdateCompletedEvent } from "../domain/interfaces/IEventService"
...
describe("SpreadsheetFacade", () => {
  ...
  describe("range operations", () => {
    ...
    test("setCellsInRange", () => {
      ...
      if (result.ok) {
        ...
        expect(facade.getCellValue(CellAddress.create(0, 1).value).value).toBe(2)
        expect(facade.getCellValue(CellAddress.create(1, 0).value).value).toBe(3)
  ...
  describe("batch operations", () => {
    ...
    test("emits batch events", () => {
      ...
      eventStore.on("BatchUpdateCompleted", (event: BatchUpdateCompletedEvent) => {
