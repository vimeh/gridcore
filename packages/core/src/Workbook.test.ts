...
describe("Workbook", () => {
  ...
  describe("sheet reordering", () => {
    ...
    test("throws error for invalid index", () => {
      ...
      expect(() => workbook.moveSheet(sheet1Id, -1)).toThrow("Invalid sheet index")
      expect(() => workbook.moveSheet(sheet1Id, 999)).toThrow("Invalid sheet index")
    })
  })
  ...
  describe("sheet duplication", () => {
    test("duplicates sheet with data", () => {
      ...
      expect(duplicated.getEngine().getCell(address)?.rawValue).toBe("Test Data")
      expect(workbook.getSheetCount()).toBe(2)
    })
    ...
    test("duplicated sheets are independent", () => {
      ...
      expect(duplicated.getEngine().getCell(address)?.rawValue).toBe("Original")
    })
