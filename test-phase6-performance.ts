...
function populateGrid(rows: number, cols: number, density: number = 0.1): void {
  console.log(`  📊 Populating grid with ${rows}x${cols} cells (${density * 100}% density)...`);
...
async function runPerformanceTests() {
  ...
  console.log(`  📊 Batch result: ${batchResult.ok ? 'Success' : 'Failed - ' + (batchResult as any).error}`);
  ...
  const memoryGrowthMB = (finalMemory.estimatedBytes - initialMemory.estimatedBytes) / 1024 / 1024;
  ...
  const passed6 = emptyResult.ok && sparseResult.ok && emptyDuration < 10 && sparseDuration < 50;
  console.log(`  ⏱️  Empty grid: ${emptyDuration.toFixed(2)}ms, Sparse grid: ${sparseDuration.toFixed(2)}ms`);
  ...
  console.log(`  💾 Current Memory: ${(gridMetrics.currentMemoryUsage / 1024 / 1024).toFixed(2)}MB`);
  
  // Overall results
  const allPassed = passed1 && passed2 && passed3 && passed4 && passed5 && passed6;
  console.log("\n🏆 PHASE 6 PERFORMANCE RESULTS:");
  console.log("=================================");
  console.log(`✅ 1000 rows insert/delete < 200ms: ${passed1 && passed2 ? 'PASS' : 'FAIL'}`);
